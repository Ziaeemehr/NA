...
! -----------------------------------------------
       ...
       ENDDO
       g_new(N) = g(i) + alph * a * (-h(1)- h(N-1)) + alph * s * h(N)
!        g_new(1) = s * h_new(1) - aa * h_new (2) - aa * h_new(N) - h_old(1)
!        DO i = 2,N-1
!           g(i) = - a * h_new(i-1) + s * h_new(i)- a * h_new(i+1)- h_old(i)
!        ENDDO
!        g(N) = -a * h_new(1) - a * h_new(N-1) + s * h_new(N)- h_old(N)
...
! -----------------------------------------------    
    DO i = 1,N
        h_old(i) = h_new(i) + sqrt(2 * D * dt) * gasdev(seed)
    ENDDO
    h_ave = 0.d0
    w = 0.d0
    DO i = 1,N
